...
必须先推送与 Podspec 版本完全一致的 Git 标签，因为 Podspec 的
`spec.source` 会读取该标签。

先验证远程源码：

```sh
pod spec lint AthenaNotation.podspec --allow-warnings
```

再发布：

```sh
pod trunk push AthenaNotation.podspec --allow-warnings
```

`pod trunk push` 会再次执行 lint。已发布的版本不可用相同版本号覆盖；
修复后必须增加版本号并创建新标签。
...
5. `pod spec lint`
6. `pod trunk push`
7. 在一个空白 iPad App 中分别验证 SPM 和 CocoaPods 安装
...
- [CocoaPods Specs 与发布](https://guides.cocoapods.org/making/specs-and-specs-repo.html)
